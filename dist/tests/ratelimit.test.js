...
describe('Rate Limiter & Security Edge Cases', () => {
    ...
    beforeAll(async () => {
        ...
        for (let i = 0; i < 100; i++) {
            const u = await client_1.prisma.user.create({
                data: {
                    email: `ratelimit_${i}_${crypto_1.default.randomBytes(4).toString('hex')}@example.com`,
                    passwordHash: 'dummy',
    ...
    afterAll(async () => {
        await client_1.prisma.user.deleteMany({ where: { id: { in: userIds } } });
    ...
    describe('Rate Limiter', () => {
        it('Should hard-stop at 100 reqs/min for a single API key when hammered with 500 requests per second', async () => {
            ...
            const promises = [];
            for (let i = 0; i < 500; i++) {
                promises.push((0, supertest_1.default)(app_1.default).get('/api/webhooks').set('x-api-key', key));
            }
    ...
    describe('Security Constraints (HMAC & Replay)', () => {
        ...
        it('Should accurately verify a legitimate request signed with HMAC', () => {
            ...
            const signature = crypto_1.default.createHmac('sha256', mockSecret).update(payloadString).digest('hex');
            ...
            const tamperedSignature = crypto_1.default.createHmac('sha256', mockSecret).update(tamperedPayloadString).digest('hex');
